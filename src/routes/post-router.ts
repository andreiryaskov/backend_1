...
postsRouter.get('/', (req: Request, res: Response) => {
    const getAllPosts = postsRepositories.getAllPosts()
...
postsRouter.post('/',
    ...
    const createNewPost = postsRepositories.createNewPost(
        title,
        shortDescription,
        content,
        blogId
...
postsRouter.get('/:id',(req: Request, res: Response) => {
    ...
    const getPostById = postsRepositories.getPostById(id)
    if (getPostById) {
...
postsRouter.put('/:id',
    ...
    const content = req.body.content
    const blogId = req.body.blogId
    const updatePostById = postsRepositories.updatePostById(
        id,
        title,
        shortDescription,
        content,
...
postsRouter.delete('/:id',
    ...
    (req: Request, res: Response) => {
    const id = req.params.id
    const deletePostById = postsRepositories.deletePostById(id)
    if (deletePostById) {
        return res.status(204).send()
    } else {
        return res.status(404).send()
